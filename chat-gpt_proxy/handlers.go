...
func nightmare(c *gin.Context) {
	...
	if original_request.ConversationId == nil {
		chatgpt.GenTitle(continue_info.ConversationID, continue_info.MessageID, token)
	}

	if !original_request.Stream {
		c.JSON(200, official_types.NewChatCompletion(full_response, continue_info.ConversationID, continue_info.ParentMessageID, continue_info.MessageID))
	} else {
