...
const getJwtUser = (token: string): TUser => {
  ...
  return {
    ...
    email: email,
    name: email,
    avatar: 'https://i.ibb.co/XCfrnHv/ljagushonok-pepe-64x61.png',
    role: '',
    provider: '',
    plugins: [],
    createdAt: '',
...
const AuthContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const setUserContext = useCallback(
    (userContext: TUserContext) => {
      const { token, isAuthenticated, redirect } = userContext;
      const user = token && getJwtUser(token);
      if (user) {
        setUser(user);
      }
      setToken(token);
  ...
  const login = (data: TLoginUser) => {
    loginUser.mutate(data, {
      onSuccess: (data: TLoginResponse) => {
        const { token } = data;
        setUserContext({ token, isAuthenticated: true, redirect: '/chat/new' });
      },
      onError: (error) => {
  ...
  useEffect(() => {
    ...
    if (!token || !isAuthenticated) {
      ...
      if (tokenFromCookie) {
        setUserContext({ token: tokenFromCookie, isAuthenticated: true });
      } else {
        navigate('/login');
      }
    }
  }, [token, isAuthenticated, error, navigate, setUserContext]);
