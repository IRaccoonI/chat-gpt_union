...
export const useGetMessagesByConvoId = (
  ...
  return useQuery<t.TMessage[]>(
    ...
    async () => {
      ...
      const response = await dataService.getMessagesByConvoId(id); 

      response.sort((m1, m2) => m1.createTime - m2.createTime)

      const ROOT_MESSAGE_ID = '00000000-0000-0000-0000-000000000000'
      const getMessageById = (id: string) => {
        return response.find(m => m.messageId === id)
      }
      const getMessagesWithParent = (message: t.TMessage) => {
        return response.filter(m => m.parentMessageId === message.messageId)
      }
      const sortedMessages: t.TMessage[] = []

      const fillArr = (id: string) => {
        const currentMessage = getMessageById(id)
        if (!currentMessage) {
          return
        }

        sortedMessages.push(currentMessage)

        const children = getMessagesWithParent(currentMessage)
        children.map(m => fillArr(m.messageId))
      }

      fillArr(ROOT_MESSAGE_ID)

      return sortedMessages
        .filter(m => ["User", "Assistant"].includes(m.sender))
        .map(m => m.sender === "User" ? {...m, isCreatedByUser: true} : m);
    },
    {
      refetchOnWindowFocus: false,
      refetchOnReconnect: false,
...
export const useGetSearchEnabledQuery = (
  ...
  return useQuery<boolean>([QueryKeys.searchEnabled], () => true, {
    refetchOnWindowFocus: false,
    refetchOnReconnect: false,
    refetchOnMount: false,
    ...config
